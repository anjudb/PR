function [means,sigma,err,priors]= em_gaussian_diagnol(number_of_gaussians,data)
...
diagnol_mul=eye(dim_of_feature_vector,dim_of_feature_vector);
...
for j=1:length_of_responsibily
  ...
  new_sigma(:,:,k)=new_sigma(:,:,k)./no_of_elements(1,k);
   new_sigma(:,:,k)=new_sigma(:,:,k).*diagnol_mul;
